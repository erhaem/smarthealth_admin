            ...
            <tbody v-for="(data, index) in dokterKeahlian" :key="index">
              <template v-if="data.user.idRole == 'RO-2003062'">
                <tr>
                  ...
                  <td v-else>data ga ada</td>
                  <td v-if="data.keahlianId">
                    {{ data.keahlianId.namaKeahlian }}
                  </td>
                  <td v-else>data tidak ada</td>
                  <td>
...
export default {
  ...
  created() {
    this.getSpesialis(), this.getDokter(), this.getKeahlianDokter(), this.getKeahlian()
  },
  methods: {
    getKeahlianDokter() {
      let type = 'getData'
      let url = ['master/ahli/keahlian/master', {}]
      this.isLoading = true
      this.$store
        .dispatch(type, url)
        .then((result) => {
    ...
    getSpesialis() {
      let type = 'getData'
      let url = ['master/penyakit/spesialis_penyakit']
      this.isLoading = true
      this.$store
        .dispatch(type, url)
        .then((result) => {
    ...
    deleteData() {
      ...
      let type = 'deleteData'
      let urls = this.selected.map((idDokterKeahlian) => [
    ...
    postKeahlian() {
      let type = 'postData'
      let url = ['master/ahli/keahlian/master', this.form]
      this.$store
        .dispatch(type, url)
        .then((result) => {
          if (!result.success) {
