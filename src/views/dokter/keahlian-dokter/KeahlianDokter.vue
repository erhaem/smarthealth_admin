...
export default {
  ...
  methods: {
    getKeahlian() {
      this.isLoading = true
      const params = [`page=${this.pagination.page}`, `per_page=${this.pagination.perPage}`].join(
        '&'
      )
    ...
    postKeahlian() {
      ...
      if (file && allowedFormats.includes(file.type)) {
        if (file.size <= maxSizeInBytes) {
          ...
          this.$store
            /**
             * 17/11/23 - ini nih biang keroknya: penempatan url sama formdata salah,
             * skrg dh fix
             * */
            .dispatch(type, ['/master/keahlian', formData])
            .then((result) => {
              iziToast.success({
                title: 'Success',
                position: 'topRight',
